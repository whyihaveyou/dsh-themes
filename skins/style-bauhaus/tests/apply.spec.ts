...
afterEach(async () => {
  ...
  delete document.body.dataset.dshStyleBauhaus
  document.title = ''
})

describe('包豪斯 Bauhaus skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshStyleBauhaus).toBe('')
    expect(document.title).toBe('包豪斯 Bauhaus')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshStyleBauhaus).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 包豪斯 Bauhaus'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 包豪斯 Bauhaus')
  })
})
