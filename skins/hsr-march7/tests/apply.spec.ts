...
afterEach(async () => {
  ...
  delete document.body.dataset.dshHsrMarch7
  document.title = ''
})

describe('崩坏星穹铁道 · 三月七 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshHsrMarch7).toBe('')
    expect(document.title).toBe('崩坏星穹铁道 · 三月七')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshHsrMarch7).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 崩坏星穹铁道 · 三月七'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 崩坏星穹铁道 · 三月七')
  })
})
