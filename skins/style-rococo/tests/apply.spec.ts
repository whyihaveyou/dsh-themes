...
afterEach(async () => {
  ...
  delete document.body.dataset.dshStyleRococo
  document.title = ''
})

describe('洛可可 Rococo skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshStyleRococo).toBe('')
    expect(document.title).toBe('洛可可 Rococo')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshStyleRococo).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 洛可可 Rococo'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 洛可可 Rococo')
  })
})
