...
afterEach(async () => {
  ...
  delete document.body.dataset.dshStyleNeoChinese
  document.title = ''
})

describe('新中式 Neo-Chinese skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshStyleNeoChinese).toBe('')
    expect(document.title).toBe('新中式 Neo-Chinese')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshStyleNeoChinese).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 新中式 Neo-Chinese'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 新中式 Neo-Chinese')
  })
})
