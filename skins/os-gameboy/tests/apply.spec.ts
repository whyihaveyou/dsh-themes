...
afterEach(async () => {
  ...
  delete document.body.dataset.dshOsGameboy
  document.title = ''
})

describe('Game Boy skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshOsGameboy).toBe('')
    expect(document.title).toBe('Game Boy')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshOsGameboy).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — Game Boy'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — Game Boy')
  })
})
