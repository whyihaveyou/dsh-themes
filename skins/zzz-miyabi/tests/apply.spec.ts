...
afterEach(async () => {
  ...
  delete document.body.dataset.dshZzzMiyabi
  document.title = ''
})

describe('绝区零 · 星见雅 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshZzzMiyabi).toBe('')
    expect(document.title).toBe('绝区零 · 星见雅')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshZzzMiyabi).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 绝区零 · 星见雅'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 绝区零 · 星见雅')
  })
})
