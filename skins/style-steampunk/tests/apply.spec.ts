...
afterEach(async () => {
  ...
  delete document.body.dataset.dshStyleSteampunk
  document.title = ''
})

describe('蒸汽朋克 Steampunk skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshStyleSteampunk).toBe('')
    expect(document.title).toBe('蒸汽朋克 Steampunk')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshStyleSteampunk).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 蒸汽朋克 Steampunk'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 蒸汽朋克 Steampunk')
  })
})
