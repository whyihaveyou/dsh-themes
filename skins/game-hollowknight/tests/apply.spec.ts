...
afterEach(async () => {
  ...
  delete document.body.dataset.dshGameHollowknight
  document.title = ''
})

describe('空洞骑士 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshGameHollowknight).toBe('')
    expect(document.title).toBe('空洞骑士')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshGameHollowknight).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 空洞骑士'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 空洞骑士')
  })
})
