...
afterEach(async () => {
  ...
  delete document.body.dataset.dshPokemonCharizard
  document.title = ''
})

describe('宝可梦 · 喷火龙 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshPokemonCharizard).toBe('')
    expect(document.title).toBe('宝可梦 · 喷火龙')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshPokemonCharizard).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 宝可梦 · 喷火龙'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 宝可梦 · 喷火龙')
  })
})
