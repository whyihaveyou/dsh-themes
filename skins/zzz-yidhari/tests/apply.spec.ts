...
afterEach(async () => {
  ...
  delete document.body.dataset.dshZzzYidhari
  document.title = ''
})

describe('绝区零 · 伊德海莉 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshZzzYidhari).toBe('')
    expect(document.title).toBe('绝区零 · 伊德海莉')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshZzzYidhari).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 绝区零 · 伊德海莉'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 绝区零 · 伊德海莉')
  })
})
