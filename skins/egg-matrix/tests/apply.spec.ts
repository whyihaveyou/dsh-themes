...
afterEach(async () => {
  ...
  delete document.body.dataset.dshEggMatrix
  document.title = ''
})

describe('黑客帝国 Matrix skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshEggMatrix).toBe('')
    expect(document.title).toBe('黑客帝国 Matrix')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshEggMatrix).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 黑客帝国 Matrix'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 黑客帝国 Matrix')
  })
})
