...
afterEach(async () => {
  ...
  delete document.body.dataset.dshZzzPulchra
  document.title = ''
})

describe('绝区零 · 波可娜 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshZzzPulchra).toBe('')
    expect(document.title).toBe('绝区零 · 波可娜')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshZzzPulchra).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 绝区零 · 波可娜'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 绝区零 · 波可娜')
  })
})
