...
afterEach(async () => {
  ...
  delete document.body.dataset.dshStyleDarkAcademia
  document.title = ''
})

describe('暗黑学术 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshStyleDarkAcademia).toBe('')
    expect(document.title).toBe('暗黑学术')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshStyleDarkAcademia).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 暗黑学术'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 暗黑学术')
  })
})
