...
afterEach(async () => {
  ...
  delete document.body.dataset.dshStreetFighter
  document.title = ''
})

describe('街头霸王 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshStreetFighter).toBe('')
    expect(document.title).toBe('街头霸王')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshStreetFighter).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 街头霸王'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 街头霸王')
  })
})
