...
afterEach(async () => {
  ...
  delete document.body.dataset.dshGenshinNahida
  document.title = ''
})

describe('原神 · 纳西妲 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshGenshinNahida).toBe('')
    expect(document.title).toBe('原神 · 纳西妲')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshGenshinNahida).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 原神 · 纳西妲'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 原神 · 纳西妲')
  })
})
