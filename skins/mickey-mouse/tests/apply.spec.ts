...
afterEach(async () => {
  ...
  delete document.body.dataset.dshMickeyMouse
  document.title = ''
})

describe('米老鼠 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshMickeyMouse).toBe('')
    expect(document.title).toBe('米老鼠')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshMickeyMouse).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 米老鼠'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 米老鼠')
  })
})
