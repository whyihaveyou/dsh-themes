...
afterEach(async () => {
  ...
  delete document.body.dataset.dshGameCyberpunk2077
  document.title = ''
})

describe('赛博朋克2077 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshGameCyberpunk2077).toBe('')
    expect(document.title).toBe('赛博朋克2077')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshGameCyberpunk2077).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 赛博朋克2077'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 赛博朋克2077')
  })
})
