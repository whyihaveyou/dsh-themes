...
afterEach(async () => {
  ...
  delete document.body.dataset.dshPokemonGengar
  document.title = ''
})

describe('宝可梦 · 耿鬼 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshPokemonGengar).toBe('')
    expect(document.title).toBe('宝可梦 · 耿鬼')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshPokemonGengar).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 宝可梦 · 耿鬼'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 宝可梦 · 耿鬼')
  })
})
