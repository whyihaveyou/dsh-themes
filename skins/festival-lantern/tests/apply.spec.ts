...
afterEach(async () => {
  ...
  delete document.body.dataset.dshFestivalLantern
  document.title = ''
})

describe('元宵 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshFestivalLantern).toBe('')
    expect(document.title).toBe('元宵')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshFestivalLantern).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 元宵'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 元宵')
  })
})
