...
afterEach(async () => {
  ...
  delete document.body.dataset.dshStyleGalaxy
  document.title = ''
})

describe('星云 Galaxy skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshStyleGalaxy).toBe('')
    expect(document.title).toBe('星云 Galaxy')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshStyleGalaxy).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 星云 Galaxy'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 星云 Galaxy')
  })
})
