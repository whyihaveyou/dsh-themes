...
afterEach(async () => {
  ...
  delete document.body.dataset.dshGameKirby
  document.title = ''
})

describe('星之卡比 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshGameKirby).toBe('')
    expect(document.title).toBe('星之卡比')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshGameKirby).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 星之卡比'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 星之卡比')
  })
})
