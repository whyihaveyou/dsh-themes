...
afterEach(async () => {
  ...
  delete document.body.dataset.dshOsAero
  document.title = ''
})

describe('Windows Vista Aero skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshOsAero).toBe('')
    expect(document.title).toBe('Windows Vista Aero')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshOsAero).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — Windows Vista Aero'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — Windows Vista Aero')
  })
})
