...
afterEach(async () => {
  ...
  delete document.body.dataset.dshFestivalNewyear
  document.title = ''
})

describe('跨年夜 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshFestivalNewyear).toBe('')
    expect(document.title).toBe('跨年夜')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshFestivalNewyear).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 跨年夜'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 跨年夜')
  })
})
