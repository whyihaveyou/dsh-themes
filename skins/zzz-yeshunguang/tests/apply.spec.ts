...
afterEach(async () => {
  ...
  delete document.body.dataset.dshZzzYeshunguang
  document.title = ''
})

describe('绝区零 · 叶瞬光 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshZzzYeshunguang).toBe('')
    expect(document.title).toBe('绝区零 · 叶瞬光')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshZzzYeshunguang).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 绝区零 · 叶瞬光'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 绝区零 · 叶瞬光')
  })
})
