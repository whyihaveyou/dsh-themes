...
afterEach(async () => {
  ...
  delete document.body.dataset.dshStyleVaporwave
  document.title = ''
})

describe('蒸汽波 Vaporwave skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshStyleVaporwave).toBe('')
    expect(document.title).toBe('蒸汽波 Vaporwave')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshStyleVaporwave).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 蒸汽波 Vaporwave'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 蒸汽波 Vaporwave')
  })
})
