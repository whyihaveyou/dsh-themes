...
afterEach(async () => {
  ...
  delete document.body.dataset.dshMarioYoshi
  document.title = ''
})

describe('超级马里奥 · 耀西 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshMarioYoshi).toBe('')
    expect(document.title).toBe('超级马里奥 · 耀西')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshMarioYoshi).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 超级马里奥 · 耀西'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 超级马里奥 · 耀西')
  })
})
