...
afterEach(async () => {
  ...
  delete document.body.dataset.dshStyleInkWash
  document.title = ''
})

describe('水墨 Ink Wash skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshStyleInkWash).toBe('')
    expect(document.title).toBe('水墨 Ink Wash')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshStyleInkWash).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 水墨 Ink Wash'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 水墨 Ink Wash')
  })
})
