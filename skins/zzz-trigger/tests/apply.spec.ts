...
afterEach(async () => {
  ...
  delete document.body.dataset.dshZzzTrigger
  document.title = ''
})

describe('绝区零 · 扳机 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshZzzTrigger).toBe('')
    expect(document.title).toBe('绝区零 · 扳机')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshZzzTrigger).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 绝区零 · 扳机'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 绝区零 · 扳机')
  })
})
