...
afterEach(async () => {
  ...
  delete document.body.dataset.dshStyleDunhuang
  document.title = ''
})

describe('敦煌壁画 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshStyleDunhuang).toBe('')
    expect(document.title).toBe('敦煌壁画')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshStyleDunhuang).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 敦煌壁画'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 敦煌壁画')
  })
})
