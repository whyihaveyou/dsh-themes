...
afterEach(async () => {
  ...
  delete document.body.dataset.dshNord
  document.title = ''
})

describe('Nord skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshNord).toBe('')
    expect(document.title).toBe('Nord')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshNord).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — Nord'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — Nord')
  })
})
