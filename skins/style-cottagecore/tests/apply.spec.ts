...
afterEach(async () => {
  ...
  delete document.body.dataset.dshStyleCottagecore
  document.title = ''
})

describe('田园 Cottagecore skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshStyleCottagecore).toBe('')
    expect(document.title).toBe('田园 Cottagecore')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshStyleCottagecore).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 田园 Cottagecore'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 田园 Cottagecore')
  })
})
