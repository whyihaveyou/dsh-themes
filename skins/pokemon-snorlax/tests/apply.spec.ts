...
afterEach(async () => {
  ...
  delete document.body.dataset.dshPokemonSnorlax
  document.title = ''
})

describe('宝可梦 · 卡比兽 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshPokemonSnorlax).toBe('')
    expect(document.title).toBe('宝可梦 · 卡比兽')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshPokemonSnorlax).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 宝可梦 · 卡比兽'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 宝可梦 · 卡比兽')
  })
})
