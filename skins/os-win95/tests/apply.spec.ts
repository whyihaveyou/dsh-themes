...
afterEach(async () => {
  ...
  delete document.body.dataset.dshOsWin95
  document.title = ''
})

describe('Windows 95 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshOsWin95).toBe('')
    expect(document.title).toBe('Windows 95')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshOsWin95).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — Windows 95'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — Windows 95')
  })
})
