...
afterEach(async () => {
  ...
  delete document.body.dataset.dshEverforest
  document.title = ''
})

describe('Everforest skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshEverforest).toBe('')
    expect(document.title).toBe('Everforest')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshEverforest).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — Everforest'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — Everforest')
  })
})
