...
afterEach(async () => {
  ...
  delete document.body.dataset.dshMarioLuigi
  document.title = ''
})

describe('超级马里奥 · 路易吉 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshMarioLuigi).toBe('')
    expect(document.title).toBe('超级马里奥 · 路易吉')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshMarioLuigi).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 超级马里奥 · 路易吉'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 超级马里奥 · 路易吉')
  })
})
