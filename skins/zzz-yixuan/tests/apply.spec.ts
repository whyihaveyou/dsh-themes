...
afterEach(async () => {
  ...
  delete document.body.dataset.dshZzzYixuan
  document.title = ''
})

describe('绝区零 · 仪玄 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshZzzYixuan).toBe('')
    expect(document.title).toBe('绝区零 · 仪玄')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshZzzYixuan).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 绝区零 · 仪玄'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 绝区零 · 仪玄')
  })
})
