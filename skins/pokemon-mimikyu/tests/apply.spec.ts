...
afterEach(async () => {
  ...
  delete document.body.dataset.dshPokemonMimikyu
  document.title = ''
})

describe('宝可梦 · 谜拟丘 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshPokemonMimikyu).toBe('')
    expect(document.title).toBe('宝可梦 · 谜拟丘')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshPokemonMimikyu).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 宝可梦 · 谜拟丘'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 宝可梦 · 谜拟丘')
  })
})
