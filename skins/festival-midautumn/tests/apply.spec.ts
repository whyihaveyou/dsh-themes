...
afterEach(async () => {
  ...
  delete document.body.dataset.dshFestivalMidautumn
  document.title = ''
})

describe('中秋 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshFestivalMidautumn).toBe('')
    expect(document.title).toBe('中秋')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshFestivalMidautumn).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 中秋'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 中秋')
  })
})
