...
afterEach(async () => {
  ...
  delete document.body.dataset.dshRetroTerminal
  document.title = ''
})

describe('Retro Terminal Amber skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshRetroTerminal).toBe('')
    expect(document.title).toBe('Retro Terminal Amber')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshRetroTerminal).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — Retro Terminal Amber'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — Retro Terminal Amber')
  })
})
