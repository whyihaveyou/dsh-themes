...
afterEach(async () => {
  ...
  delete document.body.dataset.dshStyleY2k
  document.title = ''
})

describe('Y2K 千禧 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshStyleY2k).toBe('')
    expect(document.title).toBe('Y2K 千禧')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshStyleY2k).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — Y2K 千禧'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — Y2K 千禧')
  })
})
