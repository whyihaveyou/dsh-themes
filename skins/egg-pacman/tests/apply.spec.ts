...
afterEach(async () => {
  ...
  delete document.body.dataset.dshEggPacman
  document.title = ''
})

describe('吃豆人 Pac-Man skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshEggPacman).toBe('')
    expect(document.title).toBe('吃豆人 Pac-Man')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshEggPacman).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 吃豆人 Pac-Man'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 吃豆人 Pac-Man')
  })
})
