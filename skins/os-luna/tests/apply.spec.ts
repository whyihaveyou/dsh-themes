...
afterEach(async () => {
  ...
  delete document.body.dataset.dshOsLuna
  document.title = ''
})

describe('Windows XP Luna skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshOsLuna).toBe('')
    expect(document.title).toBe('Windows XP Luna')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshOsLuna).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — Windows XP Luna'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — Windows XP Luna')
  })
})
