...
afterEach(async () => {
  ...
  delete document.body.dataset.dshZzzKoleda
  document.title = ''
})

describe('绝区零 · 珂蕾妲 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshZzzKoleda).toBe('')
    expect(document.title).toBe('绝区零 · 珂蕾妲')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshZzzKoleda).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 绝区零 · 珂蕾妲'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 绝区零 · 珂蕾妲')
  })
})
