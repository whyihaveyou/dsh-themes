...
afterEach(async () => {
  ...
  delete document.body.dataset.dshGamePortal
  document.title = ''
})

describe('传送门 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshGamePortal).toBe('')
    expect(document.title).toBe('传送门')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshGamePortal).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 传送门'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 传送门')
  })
})
