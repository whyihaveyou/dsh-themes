...
afterEach(async () => {
  ...
  delete document.body.dataset.dshSolarized
  document.title = ''
})

describe('Solarized Light skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshSolarized).toBe('')
    expect(document.title).toBe('Solarized Light')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshSolarized).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — Solarized Light'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — Solarized Light')
  })
})
