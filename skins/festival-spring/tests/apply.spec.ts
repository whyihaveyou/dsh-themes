...
afterEach(async () => {
  ...
  delete document.body.dataset.dshFestivalSpring
  document.title = ''
})

describe('春节 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshFestivalSpring).toBe('')
    expect(document.title).toBe('春节')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshFestivalSpring).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 春节'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 春节')
  })
})
