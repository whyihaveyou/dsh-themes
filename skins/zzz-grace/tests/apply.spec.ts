...
afterEach(async () => {
  ...
  delete document.body.dataset.dshZzzGrace
  document.title = ''
})

describe('绝区零 · 格莉丝 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshZzzGrace).toBe('')
    expect(document.title).toBe('绝区零 · 格莉丝')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshZzzGrace).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 绝区零 · 格莉丝'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 绝区零 · 格莉丝')
  })
})
