...
afterEach(async () => {
  ...
  delete document.body.dataset.dshOsMacosAqua
  document.title = ''
})

describe('macOS Aqua skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshOsMacosAqua).toBe('')
    expect(document.title).toBe('macOS Aqua')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshOsMacosAqua).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — macOS Aqua'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — macOS Aqua')
  })
})
