...
afterEach(async () => {
  ...
  delete document.body.dataset.dshStyleDoodle
  document.title = ''
})

describe('手绘涂鸦 Doodle skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshStyleDoodle).toBe('')
    expect(document.title).toBe('手绘涂鸦 Doodle')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshStyleDoodle).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 手绘涂鸦 Doodle'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 手绘涂鸦 Doodle')
  })
})
