...
afterEach(async () => {
  ...
  delete document.body.dataset.dshPalenight
  document.title = ''
})

describe('Material Theme Lighter skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshPalenight).toBe('')
    expect(document.title).toBe('Material Theme Lighter')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshPalenight).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — Material Theme Lighter'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — Material Theme Lighter')
  })
})
