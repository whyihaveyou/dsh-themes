...
afterEach(async () => {
  ...
  delete document.body.dataset.dshGameTetris
  document.title = ''
})

describe('俄罗斯方块 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshGameTetris).toBe('')
    expect(document.title).toBe('俄罗斯方块')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshGameTetris).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 俄罗斯方块'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 俄罗斯方块')
  })
})
