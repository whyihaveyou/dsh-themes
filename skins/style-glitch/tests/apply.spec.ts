...
afterEach(async () => {
  ...
  delete document.body.dataset.dshStyleGlitch
  document.title = ''
})

describe('故障艺术 Glitch skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshStyleGlitch).toBe('')
    expect(document.title).toBe('故障艺术 Glitch')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshStyleGlitch).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 故障艺术 Glitch'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 故障艺术 Glitch')
  })
})
