...
afterEach(async () => {
  ...
  delete document.body.dataset.dshFestivalHalloween
  document.title = ''
})

describe('万圣节 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshFestivalHalloween).toBe('')
    expect(document.title).toBe('万圣节')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshFestivalHalloween).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 万圣节'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 万圣节')
  })
})
