...
afterEach(async () => {
  ...
  delete document.body.dataset.dshHsrDanheng
  document.title = ''
})

describe('崩坏星穹铁道 · 丹恒 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshHsrDanheng).toBe('')
    expect(document.title).toBe('崩坏星穹铁道 · 丹恒')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshHsrDanheng).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 崩坏星穹铁道 · 丹恒'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 崩坏星穹铁道 · 丹恒')
  })
})
