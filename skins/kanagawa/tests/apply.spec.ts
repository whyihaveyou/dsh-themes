...
afterEach(async () => {
  ...
  delete document.body.dataset.dshKanagawa
  document.title = ''
})

describe('Kanagawa Lotus skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshKanagawa).toBe('')
    expect(document.title).toBe('Kanagawa Lotus')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshKanagawa).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — Kanagawa Lotus'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — Kanagawa Lotus')
  })
})
