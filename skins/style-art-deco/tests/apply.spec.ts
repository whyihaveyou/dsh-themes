...
afterEach(async () => {
  ...
  delete document.body.dataset.dshStyleArtDeco
  document.title = ''
})

describe('Art Deco 装饰艺术 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshStyleArtDeco).toBe('')
    expect(document.title).toBe('Art Deco 装饰艺术')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshStyleArtDeco).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — Art Deco 装饰艺术'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — Art Deco 装饰艺术')
  })
})
