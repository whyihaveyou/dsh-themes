...
afterEach(async () => {
  ...
  delete document.body.dataset.dshFestivalChristmas
  document.title = ''
})

describe('圣诞节 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshFestivalChristmas).toBe('')
    expect(document.title).toBe('圣诞节')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshFestivalChristmas).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 圣诞节'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 圣诞节')
  })
})
