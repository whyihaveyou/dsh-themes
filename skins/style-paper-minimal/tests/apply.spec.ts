...
afterEach(async () => {
  ...
  delete document.body.dataset.dshStylePaperMinimal
  document.title = ''
})

describe('极简纸 Paper Minimal skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshStylePaperMinimal).toBe('')
    expect(document.title).toBe('极简纸 Paper Minimal')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshStylePaperMinimal).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 极简纸 Paper Minimal'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 极简纸 Paper Minimal')
  })
})
