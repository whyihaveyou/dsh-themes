...
afterEach(async () => {
  ...
  delete document.body.dataset.dshGenshinWanderer
  document.title = ''
})

describe('原神 · 散兵 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshGenshinWanderer).toBe('')
    expect(document.title).toBe('原神 · 散兵')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshGenshinWanderer).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 原神 · 散兵'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 原神 · 散兵')
  })
})
