...
afterEach(async () => {
  ...
  delete document.body.dataset.dshRosePine
  document.title = ''
})

describe('Rosé Pine Dawn skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshRosePine).toBe('')
    expect(document.title).toBe('Rosé Pine Dawn')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshRosePine).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — Rosé Pine Dawn'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — Rosé Pine Dawn')
  })
})
