...
afterEach(async () => {
  ...
  delete document.body.dataset.dshHsrKafka
  document.title = ''
})

describe('崩坏星穹铁道 · 卡芙卡 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshHsrKafka).toBe('')
    expect(document.title).toBe('崩坏星穹铁道 · 卡芙卡')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshHsrKafka).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 崩坏星穹铁道 · 卡芙卡'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 崩坏星穹铁道 · 卡芙卡')
  })
})
