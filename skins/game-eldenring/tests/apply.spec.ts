...
afterEach(async () => {
  ...
  delete document.body.dataset.dshGameEldenring
  document.title = ''
})

describe('艾尔登法环 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshGameEldenring).toBe('')
    expect(document.title).toBe('艾尔登法环')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshGameEldenring).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 艾尔登法环'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 艾尔登法环')
  })
})
