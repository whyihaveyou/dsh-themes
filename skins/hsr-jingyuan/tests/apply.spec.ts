...
afterEach(async () => {
  ...
  delete document.body.dataset.dshHsrJingyuan
  document.title = ''
})

describe('崩坏星穹铁道 · 景元 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshHsrJingyuan).toBe('')
    expect(document.title).toBe('崩坏星穹铁道 · 景元')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshHsrJingyuan).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 崩坏星穹铁道 · 景元'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 崩坏星穹铁道 · 景元')
  })
})
