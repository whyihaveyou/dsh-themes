...
afterEach(async () => {
  ...
  delete document.body.dataset.dshPokemonMewtwo
  document.title = ''
})

describe('宝可梦 · 超梦 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshPokemonMewtwo).toBe('')
    expect(document.title).toBe('宝可梦 · 超梦')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshPokemonMewtwo).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 宝可梦 · 超梦'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 宝可梦 · 超梦')
  })
})
