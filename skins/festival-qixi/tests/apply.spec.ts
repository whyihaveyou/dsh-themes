...
afterEach(async () => {
  ...
  delete document.body.dataset.dshFestivalQixi
  document.title = ''
})

describe('七夕 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshFestivalQixi).toBe('')
    expect(document.title).toBe('七夕')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshFestivalQixi).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 七夕'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 七夕')
  })
})
