...
afterEach(async () => {
  ...
  delete document.body.dataset.dshHsrAcheron
  document.title = ''
})

describe('崩坏星穹铁道 · 黄泉 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshHsrAcheron).toBe('')
    expect(document.title).toBe('崩坏星穹铁道 · 黄泉')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshHsrAcheron).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 崩坏星穹铁道 · 黄泉'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 崩坏星穹铁道 · 黄泉')
  })
})
