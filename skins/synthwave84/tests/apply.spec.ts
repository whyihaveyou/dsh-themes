...
afterEach(async () => {
  ...
  delete document.body.dataset.dshSynthwave84
  document.title = ''
})

describe("Synthwave '84" + ' skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshSynthwave84).toBe('')
    expect(document.title).toBe("Synthwave '84")
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshSynthwave84).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — ' + "Synthwave '84"
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — ' + "Synthwave '84")
  })
})
