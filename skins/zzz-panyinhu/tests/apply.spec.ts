...
afterEach(async () => {
  ...
  delete document.body.dataset.dshZzzPanyinhu
  document.title = ''
})

describe('绝区零 · 潘引壶 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshZzzPanyinhu).toBe('')
    expect(document.title).toBe('绝区零 · 潘引壶')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshZzzPanyinhu).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 绝区零 · 潘引壶'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 绝区零 · 潘引壶')
  })
})
