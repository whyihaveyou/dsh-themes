...
afterEach(async () => {
  ...
  delete document.body.dataset.dshTokyoNight
  document.title = ''
})

describe('Tokyo Night Day skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshTokyoNight).toBe('')
    expect(document.title).toBe('Tokyo Night Day')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshTokyoNight).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — Tokyo Night Day'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — Tokyo Night Day')
  })
})
