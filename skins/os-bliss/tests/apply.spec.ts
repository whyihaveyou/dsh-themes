...
afterEach(async () => {
  ...
  delete document.body.dataset.dshOsBliss
  document.title = ''
})

describe('Windows XP Bliss skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshOsBliss).toBe('')
    expect(document.title).toBe('Windows XP Bliss')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshOsBliss).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — Windows XP Bliss'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — Windows XP Bliss')
  })
})
