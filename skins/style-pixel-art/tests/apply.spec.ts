...
afterEach(async () => {
  ...
  delete document.body.dataset.dshStylePixelArt
  document.title = ''
})

describe('像素艺术 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshStylePixelArt).toBe('')
    expect(document.title).toBe('像素艺术')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshStylePixelArt).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 像素艺术'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 像素艺术')
  })
})
