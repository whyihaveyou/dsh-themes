...
afterEach(async () => {
  ...
  delete document.body.dataset.dshDonaldDuck
  document.title = ''
})

describe('唐老鸭 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshDonaldDuck).toBe('')
    expect(document.title).toBe('唐老鸭')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshDonaldDuck).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 唐老鸭'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 唐老鸭')
  })
})
