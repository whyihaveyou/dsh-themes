...
afterEach(async () => {
  ...
  delete document.body.dataset.dshStyleMorandi
  document.title = ''
})

describe('莫兰迪 Morandi skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshStyleMorandi).toBe('')
    expect(document.title).toBe('莫兰迪 Morandi')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshStyleMorandi).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 莫兰迪 Morandi'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 莫兰迪 Morandi')
  })
})
