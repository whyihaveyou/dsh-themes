...
afterEach(async () => {
  ...
  delete document.body.dataset.dshStyleMemphis
  document.title = ''
})

describe('孟菲斯 Memphis skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshStyleMemphis).toBe('')
    expect(document.title).toBe('孟菲斯 Memphis')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshStyleMemphis).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 孟菲斯 Memphis'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 孟菲斯 Memphis')
  })
})
