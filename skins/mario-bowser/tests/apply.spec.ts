...
afterEach(async () => {
  ...
  delete document.body.dataset.dshMarioBowser
  document.title = ''
})

describe('超级马里奥 · 库巴 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshMarioBowser).toBe('')
    expect(document.title).toBe('超级马里奥 · 库巴')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshMarioBowser).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 超级马里奥 · 库巴'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 超级马里奥 · 库巴')
  })
})
