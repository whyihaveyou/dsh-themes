...
afterEach(async () => {
  ...
  delete document.body.dataset.dshGameAnimalcrossing
  document.title = ''
})

describe('集合啦动物森友会 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshGameAnimalcrossing).toBe('')
    expect(document.title).toBe('集合啦动物森友会')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshGameAnimalcrossing).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 集合啦动物森友会'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 集合啦动物森友会')
  })
})
