...
afterEach(async () => {
  ...
  delete document.body.dataset.dshNightOwl
  document.title = ''
})

describe('Night Owl Light skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshNightOwl).toBe('')
    expect(document.title).toBe('Night Owl Light')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshNightOwl).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — Night Owl Light'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — Night Owl Light')
  })
})
