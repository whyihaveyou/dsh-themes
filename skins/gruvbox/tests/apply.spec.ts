...
afterEach(async () => {
  ...
  delete document.body.dataset.dshGruvbox
  document.title = ''
})
...
describe('Gruvbox skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    ...
    fiber = await mount()
    expect(document.body.dataset.dshGruvbox).toBe('')
    expect(document.title).toBe('Gruvbox')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshGruvbox).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — Gruvbox'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — Gruvbox')
  })
})
