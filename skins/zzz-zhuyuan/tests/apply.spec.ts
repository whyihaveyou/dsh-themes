...
afterEach(async () => {
  ...
  delete document.body.dataset.dshZzzZhuyuan
  document.title = ''
})

describe('绝区零 · 朱鸢 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshZzzZhuyuan).toBe('')
    expect(document.title).toBe('绝区零 · 朱鸢')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshZzzZhuyuan).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 绝区零 · 朱鸢'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 绝区零 · 朱鸢')
  })
})
