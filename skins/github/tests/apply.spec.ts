...
afterEach(async () => {
  ...
  delete document.body.dataset.dshGithub
  document.title = ''
})

describe('GitHub Light skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshGithub).toBe('')
    expect(document.title).toBe('GitHub Light')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshGithub).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — GitHub Light'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — GitHub Light')
  })
})
