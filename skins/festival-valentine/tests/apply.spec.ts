...
afterEach(async () => {
  ...
  delete document.body.dataset.dshFestivalValentine
  document.title = ''
})

describe('情人节 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshFestivalValentine).toBe('')
    expect(document.title).toBe('情人节')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshFestivalValentine).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 情人节'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 情人节')
  })
})
