...
afterEach(async () => {
  ...
  delete document.body.dataset.dshGenshinHutao
  document.title = ''
})

describe('原神 · 胡桃 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshGenshinHutao).toBe('')
    expect(document.title).toBe('原神 · 胡桃')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshGenshinHutao).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 原神 · 胡桃'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 原神 · 胡桃')
  })
})
