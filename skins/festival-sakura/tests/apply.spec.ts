...
afterEach(async () => {
  ...
  delete document.body.dataset.dshFestivalSakura
  document.title = ''
})

describe('樱花季 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshFestivalSakura).toBe('')
    expect(document.title).toBe('樱花季')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshFestivalSakura).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 樱花季'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 樱花季')
  })
})
