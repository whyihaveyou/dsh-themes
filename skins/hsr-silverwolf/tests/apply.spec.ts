...
afterEach(async () => {
  ...
  delete document.body.dataset.dshHsrSilverwolf
  document.title = ''
})

describe('崩坏星穹铁道 · 银狼 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshHsrSilverwolf).toBe('')
    expect(document.title).toBe('崩坏星穹铁道 · 银狼')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshHsrSilverwolf).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 崩坏星穹铁道 · 银狼'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 崩坏星穹铁道 · 银狼')
  })
})
