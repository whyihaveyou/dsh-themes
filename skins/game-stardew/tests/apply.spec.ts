...
afterEach(async () => {
  ...
  delete document.body.dataset.dshGameStardew
  document.title = ''
})

describe('星露谷物语 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshGameStardew).toBe('')
    expect(document.title).toBe('星露谷物语')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshGameStardew).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 星露谷物语'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 星露谷物语')
  })
})
