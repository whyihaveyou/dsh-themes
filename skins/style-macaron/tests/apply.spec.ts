...
afterEach(async () => {
  ...
  delete document.body.dataset.dshStyleMacaron
  document.title = ''
})

describe('马卡龙 Macaron skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshStyleMacaron).toBe('')
    expect(document.title).toBe('马卡龙 Macaron')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshStyleMacaron).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 马卡龙 Macaron'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 马卡龙 Macaron')
  })
})
