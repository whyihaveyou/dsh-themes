...
afterEach(async () => {
  ...
  delete document.body.dataset.dshGenshinRaiden
  document.title = ''
})

describe('原神 · 雷电将军 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshGenshinRaiden).toBe('')
    expect(document.title).toBe('原神 · 雷电将军')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshGenshinRaiden).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 原神 · 雷电将军'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 原神 · 雷电将军')
  })
})
