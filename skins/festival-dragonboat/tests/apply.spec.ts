...
afterEach(async () => {
  ...
  delete document.body.dataset.dshFestivalDragonboat
  document.title = ''
})

describe('端午 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshFestivalDragonboat).toBe('')
    expect(document.title).toBe('端午')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshFestivalDragonboat).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 端午'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 端午')
  })
})
