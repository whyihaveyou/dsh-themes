...
afterEach(async () => {
  ...
  delete document.body.dataset.dshCatppuccin
  document.title = ''
})

describe('Catppuccin Latte skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshCatppuccin).toBe('')
    expect(document.title).toBe('Catppuccin Latte')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshCatppuccin).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — Catppuccin Latte'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — Catppuccin Latte')
  })
})
