...
afterEach(async () => {
  ...
  delete document.body.dataset.dshZeldaLink
  document.title = ''
})

describe('塞尔达传说 · 林克 skin apply', () => {
  it('mounts attribute, favicon and title', async () => {
    document.title = 'DeepSeek Harness'
    fiber = await mount()
    expect(document.body.dataset.dshZeldaLink).toBe('')
    expect(document.title).toBe('塞尔达传说 · 林克')
  ...
  it('retracts everything on dispose', async () => {
    ...
    await fiber.dispose()
    fiber = undefined
    expect(document.body.dataset.dshZeldaLink).toBeUndefined()
  ...
  it('never clobbers a session title', async () => {
    fiber = await mount()
    document.title = '我的会话 — 塞尔达传说 · 林克'
    await fiber.dispose()
    fiber = undefined
    expect(document.title).toBe('我的会话 — 塞尔达传说 · 林克')
  })
})
